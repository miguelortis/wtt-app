...
export class DutiesService {
  ...
  async createDuty(routeId: string, unitId: string, startTime: Date, endTime: Date) {
    ...
    try {
      ...
      const overlappingDuty = await this.dutyModel.findOne({
        unitId,
      ...
      if (overlappingDuty) {
        throw new ConflictException('La unidad ya tiene un duty asignado en este horario.');
      }
      ...
      throw new InternalServerErrorException('Error al asignar el duty por problemas de concurrencia.');
    } finally {
